...
import { generatePost as generatePostApi } from "@/app/actions/generatePost"
import { generatePostImage } from "@/app/actions/generateImage"
...
export function PostForm() {
  ...
  const useLocalMockData = async () => {
    // Generate a mock result but use real image generation
    const mockContent = `Here's a compelling post about "${prompt}". This would typically be generated by AI, but we're using mock data for now.`;
    const mockVisualPrompt = `A beautiful visual representing ${prompt}`;
    
    // Use the real image generation function to get an actual image
    let imageUrl = null;
    try {
      imageUrl = await generatePostImage(mockContent, mockVisualPrompt, visualStyle, platform);
    } catch (error) {
      console.error("Error generating image for mock data:", error);
    }
    
    const mockResult = {
      mainContent: mockContent,
      ...
      imageUrl: imageUrl, // Use the actual generated image
      visualPrompt: mockVisualPrompt
    };
  ...
  const fetchData = async () => {
    try {
      if (process.env.NEXT_PUBLIC_USE_MOCK_DATA === 'true') {
        console.log('Using mock data (environment variable)');
        await useLocalMockData();
        return;
      }
    } catch (error) {
